...
plt.switch_backend('agg')  # <-- helps prevent backend-related issues in Streamlit

st.title("Employee Engagement RFM Dashboard")

# Load dataset
@st.cache_data  # Use caching to speed up performance
def load_data():
    return pd.read_csv('DEPLOYABLE.csv')
